	...
	function getQuote($address) {
		...
		if ($status) {
			...
			$quote_data['courier'] = array(
				...
				'text'         => $this->currency->format($this->tax->calculate($this->config->get('courier_cost'), $this->config->get('courier_tax_class_id'), $this->config->get('config_tax')), $this->session->data['currency'])
			);
