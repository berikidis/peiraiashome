/** Enhanced E-Commerce Tracking Module v2.2.1.1 -- https://vanstudio.co.ua -- */
...
$(document).ready(function() {
    $(document).on('click', '[onclick*=\'cart.add\']', function(e) {
        ...
        if (ee_data[0]) {
            $.ajax({
                ...
                type: 'post',
                data: { 'product_id': ee_data[0], 'quantity': isNaN(ee_data[1]) ? $(this).parent().parent().find('input[name=\'quantity\']').val() : ee_data[1], 'type': $(this).data('eet-type'), 'position': $(this).data('eet-position'), 'url': window.location.href, 'title': document.title },
                dataType: 'json',
                success: function(json) {
                    if (json) {
                        console.log(json);
    ...
    $('#button-cart').on('click', function() {
        ...
        $.ajax({
            ...
            type: 'post',
            data: { 'product_id': isNaN(product_id) ? $('input[name=\'product_id\']').val() : product_id, 'quantity': isNaN(quantity) ? $('input[name=\'quantity\']').val() : quantity, 'type': 'product', 'option': $("[name^=\'option\']").serializeArray(), 'url': window.location.href, 'title': document.title },
            dataType: 'json',
            success: function(json) {
                if (json) {
                    console.log(json);
