...
class ControllerFeedSkroutz extends Controller {
	...
	public function index() {
		...
		$COLOR_OPTION_ID = 13;
		$SIZE_OPTION_ID = 14;
		$SIZE_ATTRIBUTE_ID = 14;
		$CREATE_ZIP = true;

		$this->load->model('catalog/category');
		$this->load->model('catalog/product');
		$this->load->model('extension/feed/skroutz');

		$products = $this->model_extension_feed_skroutz->getProducts();
		$categories = $this->model_extension_feed_skroutz->getCategories();
		$categories_path = $this->model_extension_feed_skroutz->getCategoriesPath();
		$manufacturers = $this->model_extension_feed_skroutz->getManufacturers();
		$stock_statuses = $this->model_extension_feed_skroutz->getStockStatuses();
		$color_options = $this->model_extension_feed_skroutz->getProductOptions($COLOR_OPTION_ID);
		$size_options = $this->model_extension_feed_skroutz->getProductOptions($SIZE_OPTION_ID);
		$size_attributes = $this->model_extension_feed_skroutz->getProductAttributes($SIZE_ATTRIBUTE_ID);
		...
		foreach ($products as $product) {
			...
			if ($product['manufacturer'] == 'DIM collection' && $product['quantity'] > 0) {
				$in_stock = 'Y';
				$availability = 'Διαθέσιμο από 4 έως 10 ημέρες';
			} else if ($product['quantity'] > 0) {
				$in_stock = 'Y';
				$availability = 'Διαθέσιμο από 1 έως 3 ημέρες';
			} else {
				$in_stock = 'N';
				$availability = 'Διαθέσιμο από 10 έως 30 ημέρες'; // $stock_statuses[array_search($product['stock_status_id'], array_column($stock_statuses, 'stock_status_id'))]['name'];
			}
			...
			// check if size exist as an option
			...
			if (!$product['size']) {
				// check if size exist as an attribute
				$size_index = array_search($product['product_id'], array_column($size_attributes, 'product_id'), true);
				$product['size'] = $size_index > 0 ? $size_attributes[$size_index]['attributes'] : false;
			}
			...
			$output .= '<quantity>' . (($product['quantity'] > 0) ? $product['quantity'] : 0) . '</quantity>';
		...
		// OPEN OUTPUT FILE
		$fs = fopen("xml_feeds/skroutz.xml", "wb");
		fwrite($fs, $output);
		fclose($fs);

		// COMPRESS FILE
		if ($CREATE_ZIP) {
			$zip_file = "xml_feeds/skroutz.zip";
			if (is_file($zip_file)) unlink($zip_file);
			$zip = new ZipArchive;
			if ($zip->open("xml_feeds/skroutz.zip", ZipArchive::CREATE) === TRUE) {
				$zip->addFile("xml_feeds/skroutz.xml");
				$zip->close();
			}
		}

		$this->response->setOutput('<p>Create skroutz.xml...OK<br/>Products in skroutz XML: ' . $count . '<br/><a href="/xml_feeds/skroutz.xml" target="_blank">View XML</a> or <a href="/xml_feeds/skroutz.zip">Download Zip</a></p>');
	}
