...
class ControllerToolKaProductImport extends KaController { 
	...
	protected function onLoad() {
		...
		if (!$this->validate()) {
			return $this->redirect($this->url->link('error/permission', 'token=' . $this->session->data['token'], true));
		}

		$this->load->kamodel('tool/ka_product_import');
		
		if (!$this->db->isKaInstalled('ka_product_import')) {
			$this->addTopMessage("The extension is not installed");
			return $this->redirect($this->url->link('extension/ka_extensions', 'token=' . $this->session->data['token'], true));
		}
	...
	protected function prepareOutput() {
		...
		if (empty($this->template)) {
			$this->template = 'tool/ka_product_import';
		}
	...
	public function index() { // step1
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			...
			if (!isset($this->request->post['images_dir'])) {
				...
				$this->session->data['save_params'] = true;
			 	return $this->redirect($this->url->link('tool/ka_product_import', 'token=' . $this->session->data['token'], true));
			}
			...
			return $this->redirect($this->url->link('tool/ka_product_import/step2', 'token=' . $this->session->data['token'], true));
		}
		...
		// tpl product will be displayed on a store if the xml patch was applied to standard files
		//
		...
		$this->data['action']    = $this->url->link('tool/ka_product_import', 'token=' . $this->session->data['token'], true);
		...
		$this->data['settings_page'] = $this->url->link('ka_extensions/ka_product_import', 'token=' . $this->session->data['token'], true);
		$this->data['product_url']   = $this->url->link('catalog/product/edit', '', true) . '&token=' . $this->session->data['token'];
	...
	/*
		The function updates $this->params['matches'] array with selected columns.
		...
		RESULT:
			...
			array(9) {
			  ...
			  array(6) {
				["customer_group"]=>
				string(23) "discount:Customer Group"
				["quantity"]=>
				string(17) "discount:Quantity"
				...
			  ["attributes"]=>
			  array(11) {
				[4]=>
				string(16) "attribute:test 1"
				[5]=>
				string(16) "attribute:test 2"
        		
	*/
	protected function saveMatchesByNames() {
					...
					if (is_array($fields[$f_key])) {
						...
						foreach ($fields[$f_key] as $ffk => $ffv) {
							if ($fields[$f_key][$ffk] > 0) {
								$matches[$sk][$f_key][] = $this->params['columns'][$fields[$f_key][$ffk]-1];
							}
						}
	...
	public function step2() { // step2
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
			...
			$this->saveMatchesByNames();
			
			$sets = $this->model_tool_ka_product_import->getFieldSets();
			$this->model_tool_ka_product_import->copyMatchesToSets($sets, $this->params['matches'], $this->data['columns']);
			...
			return $this->redirect($this->url->link('tool/ka_product_import/step3', 'token=' . $this->session->data['token'], true));
		}
		...
		$this->model_tool_ka_product_import->findMatchesByNames($sets, $this->data['columns']);
		
		$this->data['matches'] = $sets;
		
		$this->data['attribute_page_url'] = $this->url->link('catalog/attribute', 'token=' . $this->session->data['token'], true);
		$this->data['filter_page_url']    = $this->url->link('catalog/filter', 'token=' . $this->session->data['token'], true);
		$this->data['option_page_url']    = $this->url->link('catalog/option', 'token=' . $this->session->data['token'], true);
    	$this->data['action']             = $this->url->link('tool/ka_product_import/step2', 'token=' . $this->session->data['token'], true);
		$this->data['back_action']        = $this->url->link('tool/ka_product_import', 'token=' . $this->session->data['token'], true);
		$this->data['params']             = $this->params;

		$this->data['filesize']           = $this->kaformat->convertToMegabyte(filesize($this->params['file']));
		$this->data['backup_link']        = $this->url->link('tool/backup', 'token=' . $this->session->data['token'], true);

		if ($this->db->isKaInstalled('ka_variants')) {
			$this->data['variants_enabled'] = true;
		}
	...
	public function step3() { // step3
		...
		$this->data['done_action']        = $this->url->link('tool/ka_product_import', 'token=' . $this->session->data['token'], true);
		$this->data['params']             = $this->params;
		...
		//
 		$this->data['page_url'] = str_replace('&amp;', '&', $this->url->link('tool/ka_product_import/stat', 'format=raw&tmpl=component&token=' . $this->session->data['token'], true));
		$this->template = 'tool/ka_product_import';
		return $this->prepareOutput();
	}
