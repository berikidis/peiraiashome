...
class ControllerExtensionShippingCourier extends Controller {
	...
	public function index() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/shipping/courier', 'token=' . $this->session->data['token'], true)
		);

		$data['action'] = $this->url->link('extension/shipping/courier', 'token=' . $this->session->data['token'], true);

		$data['cancel'] = $this->url->link('extension/extension', 'token=' . $this->session->data['token'] . '&type=shipping', true);

		if (isset($this->request->post['courier_cost'])) {
			$data['courier_cost'] = $this->request->post['courier_cost'];
		} else {
			$data['courier_cost'] = $this->config->get('courier_cost');
		}

		if (isset($this->request->post['courier_tax_class_id'])) {
			$data['courier_tax_class_id'] = $this->request->post['courier_tax_class_id'];
		} else {
			$data['courier_tax_class_id'] = $this->config->get('courier_tax_class_id');
		}
		...
		if (isset($this->request->post['courier_geo_zone_id'])) {
			$data['courier_geo_zone_id'] = $this->request->post['courier_geo_zone_id'];
		} else {
			$data['courier_geo_zone_id'] = $this->config->get('courier_geo_zone_id');
		}
