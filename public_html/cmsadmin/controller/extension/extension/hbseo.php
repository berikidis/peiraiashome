...
class ControllerExtensionExtensionHbseo extends Controller {
	...
	public function menu(){
		...
		if ($files) {
			foreach ($files as $file) {
				...
				$data['extensions'][] = array(
					'name'      => $menu_title,
					'href'   	=> $this->url->link('extension/hbseo/' . $extension, $this->hb_token_name.'=' . $this->session->data[$this->hb_token_name] . '&store_id=0', true)
				);
		...
		if ($this->user->hasPermission('access', 'extension/extension/hbseo')) {
	...
	public function install() {
		...
		if ($this->validate()) {
			if (version_compare(VERSION,'3.0.0.0','<' )) {
				...
				$this->model_setting_extension->install('hbseo', $this->request->get['extension']);
			}
	...
	public function uninstall() {
		...
		if ($this->validate()) {
			if (version_compare(VERSION,'3.0.0.0','<' )) {
				...
				$this->model_setting_extension->uninstall('hbseo', $this->request->get['extension']);
			}
	...
	public function update() {
		...
		if ($this->validate()) {
			...
			if (!$this->load->controller('extension/hbseo/' . $this->request->get['extension'] . '/update')){
	...
	protected function getList() {
		...
		if (version_compare(VERSION,'3.0.0.0','<' )) {
			...
			$extensions = $this->model_setting_extension->getInstalled('hbseo');
		}
		...
		foreach ($extensions as $key => $value) {
			if (!is_file(DIR_APPLICATION . 'controller/extension/hbseo/' . $value . '.php') && !is_file(DIR_APPLICATION . 'controller/hbseo/' . $value . '.php')) {
				if (version_compare(VERSION,'3.0.0.0','<' )) {
					...
					$this->model_setting_extension->uninstall('hbseo', $value);
				}
		...
		$files = glob(DIR_APPLICATION . 'controller/extension/hbseo/*.php', GLOB_BRACE);
		...
		if ($files) {
			foreach ($files as $file) {
				...
				$store_data[] = array(
					...
					'edit'   => $this->url->link('extension/hbseo/' . $extension, $this->hb_token_name.'=' . $this->session->data[$this->hb_token_name] . '&store_id=0', true)
				);
									
				foreach ($stores as $store) {
					$store_data[] = array(
						'name'   => $store['name'],
						'edit'   => $this->url->link('extension/hbseo/' . $extension, $this->hb_token_name.'=' . $this->session->data[$this->hb_token_name] . '&store_id=' . $store['store_id'], true)
					);
				}
				
				$data['extensions'][] = array(
					'name'      => $this->language->get('heading_title'),
					'install'   => $this->url->link('extension/extension/hbseo/install', $this->hb_token_name.'=' . $this->session->data[$this->hb_token_name] . '&extension=' . $extension, true),
					'uninstall' => $this->url->link('extension/extension/hbseo/uninstall', $this->hb_token_name.'=' . $this->session->data[$this->hb_token_name] . '&extension=' . $extension, true),
					'update' 	=> $this->url->link('extension/extension/hbseo/update', $this->hb_token_name.'=' . $this->session->data[$this->hb_token_name] . '&extension=' . $extension, true),
