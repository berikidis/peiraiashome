...
class ControllerModuleSeoMegaPack extends Controller {
	...
	private function getSitemaps( $skipindex = false ) {
		...
		$sitemaps		= array();
		$http_server	= $this->_mainURL( true );
	...
	private function _createSitemapProducts( $store_id = 0, $page = 1, $limit = 100 ) {
		...
		foreach( $this->db->query( $sql )->rows as $product ) {
			...
			if ($product['image']) {
				...
				$product_image['img_url'] = $this->model_tool_image->resize($product['image'], $this->config->get($this->config->get('config_theme') . '_image_popup_width'), $this->config->get($this->config->get('config_theme') . '_image_popup_height'));
				}else{
					$product_image['img_url'] = $this->model_tool_image->resize($product['image'], $this->config->get('config_image_popup_width'), $this->config->get('config_image_popup_height'));
				}
				$product_image['img_name'] = $product['name'];
			}
		
			$str .= $this->_createSitemapLink( $store_id, $this->url->link( 'product/product', 'product_id=' . $product['product_id'] ), empty( $settings[$store_id]['product'] ) ? 'weekly' : $settings[$store_id]['product'], '0.5', $product_image );
		}
	...
	private function _createSitemapCategories( $store_id = 0, $page = 1, $limit = 100, $parent_id = 0, $path = '' ) {
		...
		foreach( $this->db->query( $sql )->rows as $category ) {
			...
			if ($category['image']) {
				if(version_compare( VERSION, '2.2.0.0', '>=' )){
				$category_image['img_url'] = $this->model_tool_image->resize($category['image'], $this->config->get($this->config->get('config_theme') . '_image_popup_width'), $this->config->get($this->config->get('config_theme') . '_image_popup_height'));
				}else{
					$category_image['img_url'] = $this->model_tool_image->resize($category['image'], $this->config->get('config_image_popup_width'), $this->config->get('config_image_popup_height'));
				}
				$category_image['img_name'] = $category['name'];
			}
		
			$str .= $this->_createSitemapLink( $store_id, $this->url->link( 'product/category', 'path=' . $path ), empty( $settings[$store_id]['category'] ) ? 'yearly' : $settings[$store_id]['category'], '1.0', $category_image );
		}
	 ...
	 * @return string
	 */
	private function _createSitemapLink( $store_id, $link, $changefreq, $priority, $img = null ) {
		$ssl	= (strpos( $link, 'https://' ) === 0 || (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') || $_SERVER['SERVER_PORT'] == 443) ? true : false;
		...
		if( $img ){
			$img_part .= sprintf("\r\n\t\t<image:image>\r\n\t\t<image:loc>%s</image:loc>\r\n\t\t<image:title>%s</image:title>\r\n\t\t</image:image>",
				$img['img_url'], $img['img_name']);
		}
		
		return sprintf("\t<url>\r\n\t\t<loc>%s</loc>\r\n\t\t<lastmod>%s</lastmod>\r\n\t\t<changefreq>%s</changefreq>\r\n\t\t<priority>%s</priority>".$img_part."\r\n\t</url>\r\n",
...
//			}
	...
	public function settings() {
		...
		if( $this->request->server['REQUEST_METHOD'] == 'POST' ) {	
			...
			if( !empty( $this->request->post['smp_language_code_alias'] ) ) {
				$this->load->model( 'catalog/url_alias' );
				
				$alias_errors = '';
				
				foreach( $this->request->post['smp_language_code_alias'] as $lang_idx => $seo_keyword ) {
					if( utf8_strlen( $seo_keyword ) > 0 ) {
						$url_alias_info = $this->model_catalog_url_alias->getUrlAlias( $seo_keyword );
						if( $url_alias_info ) {
							$this->request->post['smp_language_code_alias'][$lang_idx] = $seo_keyword.'_'.$lang_idx;
							if(!$alias_errors){
								$alias_errors .= sprintf( $this->language->get( 'error_keyword'), $lang_idx, $seo_keyword, $lang_idx );
							}else{
								$alias_errors .= sprintf( $this->language->get( 'error_keyword_2'), $lang_idx, $seo_keyword, $lang_idx ); 
							}
						}
					}
				}
				if($alias_errors){
					$this->session->data['error'] = $alias_errors;
				}
			}
