...
class ModelToolKaProductImport extends KaModel {
	...
	function onLoad() {
		...
		if ($this->db->isKaInstalled('ka_variants')) {
			$this->load->model('catalog/ka_variant');
		}
	...
	*/
	...
	/*
		...
		RETURNS:
			...
			false - error.
	*/
	public function findMatchesByNames(&$matches, $columns) {
		...
		*/
		...
		if ($this->db->isKaInstalled('ka_variants')) {
			$sets['variants'] = array('field', 'name', 'variant:');
		}
		...
		foreach ($sets as $sk => $sv) {
			...
			foreach ($matches[$sk] as $mk => $mv) {
				...
				$possible_names = array(
					$sv[2] . utf8_strtolower($mv[$sv[0]], 'utf-8'),					
					$sv[2] . utf8_strtolower($mv[$sv[1]], 'utf-8')
				);

				$column_indexes = $this->findColumnsByNames($columns, $possible_names, !empty($mv['can_be_cloned']));
				$mv['column'] = $column_indexes;
	...
	protected function findColumnsByNames($columns, $names, $can_be_cloned = false) {
		...
		foreach ($names as $name) {
			...
			foreach ($columns as $ck => $cv) {
				...
				} elseif ($can_be_cloned) {
					if (stripos($ck, $name . '_') === 0) {
						$column_keys[] = $columns[$ck];
					}
				}
			}
	...
	*/
	protected function updateProduct($product_id, $data, $is_new) {
		...
		if (isset($data['price']) && strlen($data['price'])) {
			$product['price'] = $this->kaformat->parsePrice($data['price']);
			...
			if (!empty($data['image'])) {
				...
				if ($file === false) {
					if (!empty($this->lastError)) {
						$this->addImportMessage($this->product_mark . "main image cannot be saved - " . $this->lastError);
					}
				} elseif (!empty($file)) {
					$product['image'] = $file;
				}
	...
	*/
	protected function updateProductDescription($product_id, $data) {
		...
		foreach ($this->params['matches']['fields'] as $field) {
			if (!empty($field['is_product_description']) && isset($data[$field['field']])) {
				$lang[$field['field']] = trim($data[$field['field']]);
			}
		}
	...
	*/
	protected function saveAdditionalImage($product_id, $additional_image) {

		// insert an additional product image
		//
		if (!empty($this->params['ka_pi_image_separator'])) {
			$images = explode($this->params['ka_pi_image_separator'], $additional_image);
		} else {
			$images = array($additional_image);
		}

		$sort_order = 0;
		$qry = $this->db->query("SELECT sort_order FROM " . DB_PREFIX . "product_image 
			WHERE product_id = '" . (int)$product_id . "' ORDER BY sort_order DESC LIMIT 1"
		);
		...
		foreach ($images as $image) {
			...
			if ($file === false) {
				$this->addImportMessage($this->product_mark . "additional image cannot be saved - " . $this->lastError);
	...
	protected function saveAdditionalImages($product_id, $data, $delete_old, $is_new) {
		...
		foreach ($data['additional_image'] as $additional_image) {
			$this->saveAdditionalImage($product_id, $additional_image);
		}
		...
		*/
		if (in_array($option['type'], $this->extended_types)) {
			...
			if (in_array($option['type'], $this->options_with_images) && !empty($option['image'])) {
				...
				if ($file === false) {
					$this->addImportMessage($this->product_mark ."option image cannot be saved - " . $this->lastError);
				} else {
					$rec['image'] = $file;
				}
			}
			...
			if (isset($option['price'])) {
				$rec['price']        = abs($this->kaformat->parsePrice($option['price']));
	...
	protected function saveDiscounts($row, $product, $delete_old) {
		...
		foreach ($this->params['matches']['discounts'] as $ak => $av) {
			...
			if ($av['field'] == 'price') {
				...
				$val = $this->kaformat->parsePrice($val);
	...
	*/
	...
	protected function saveProductProfiles($row, $product, $delete_old) {
		...
		if ($delete_old) {
			$this->db->query("DELETE FROM " . DB_PREFIX . "product_recurring WHERE product_id = '$product[product_id]'");
		}
		...
		foreach ($this->params['matches']['product_profiles'] as $ak => $av) {
			...
			if ($av['field'] == 'customer_group') {
				...
				$qry = $this->db->query("SELECT * FROM " . DB_PREFIX . "recurring_description");
				if (empty($qry->row)) {
					return false;
				}
				$data['recurring_id'] = $qry->row['recurring_id'];
				continue;
			}
		...
		$this->db->query("DELETE FROM " . DB_PREFIX . "product_recurring WHERE 
			product_id = '$product[product_id]'
			AND customer_group_id = '$data[customer_group_id]'
		");
		$this->kadb->queryInsert('product_recurring', $data);
	...
	/*
		add the option value to the specific global product option
	*/
	protected function addVariantOptionValue($option_id, $value) {
		$rec = array(
			'option_id' => $option_id,
		);
	...
	/*
		The function saves variant option-value pair and assigns it to the current product.
	
	*/	
	protected function assignVariantOptionValue($product_id, $option_id, $value) {
		...
		$qry = $this->db->query("SELECT option_value_id FROM " . DB_PREFIX . "option_value_description WHERE 
			option_id = '" . $option_id . "'
			AND name = '" . $this->db->escape($value) . "'
		");

		if (empty($qry->row['option_value_id'])) {
			$option_value_id = $this->addVariantOptionValue($option_id, $value);
		} else {
			$option_value_id = $qry->row['option_value_id'];
		}
		...
		//
	   	$qry = $this->db->query("SELECT product_option_id FROM " . DB_PREFIX . "product_option WHERE
	 		product_id='$product_id' AND option_id='$option_id'"
	 	);
		...
		$this->registerRecord($product_id, $this->record_types['product_option_id'], $product_option_id);
		
		// find product option value id or insert a new one
		//		
		$qry = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_option_value WHERE
	 		product_option_id = '$product_option_id'
     		AND option_value_id = '$option_value_id' 
		...
		$rec = array(
     	...
     	if (!empty($qry->rows)) {
     		...
     		$pov_id = (int) $qry->row['product_option_value_id'];
     	
     		if ($qry->num_rows > 1) {
				$this->db->query("DELETE FROM " . DB_PREFIX . "product_option_value WHERE
					product_option_id = '$product_option_id'
					AND option_value_id = '$option_value_id'
					AND product_option_value_id <> '" . $pov_id . "'
				");
			}
     		
		} else {
			$pov_id = $this->kadb->queryInsert('product_option_value', $rec);
		}
		$this->registerRecord($product_id, $this->record_types['product_option_value_id'], $pov_id);
		
		return $pov_id;
	}

			
	protected function saveVariants($row, $product, $delete_old, $is_first) {

		if (empty($this->params['matches']['variants'])) {
			return true;
		}

		// delete all previous variant records for this product (with their options and option values)
		//
		if ($is_first && $this->params['update_mode'] == 'replace') {		
			$this->db->query("DELETE FROM " . DB_PREFIX . "product_option po
				INNER JOIN " . DB_PREFIX . "option o ON po.option_id = o.option_id
				WHERE product_id = '" . $product['product_id']. "'
				AND is_variant = 1
			");
			
			$this->db->query("DELETE FROM " . DB_PREFIX . "product_option_value pov
				INNER JOIN " . DB_PREFIX . "option o ON po.option_id = o.option_id
				WHERE product_id = '" . $product['product_id']. "'
				AND is_variant = 1
			");
		...
		$cols = &$this->params['matches']['variants'];

		// get variant options and try to find a corresponding variant
		//
		$rec = array();
		foreach ($cols as $col) {
			if (empty($col['is_option']) || !isset($col['column'])) {
				continue;
			}
			
			$vov_id = $this->assignVariantOptionValue($product['product_id'], $col['field'], $row[$col['column']]);
		...
		if (empty($rec)) {
			$this->addImportMessage($this->product_mark . "Unable to get a variant record for variant data for productid=" . $product['product_id']);
			return false;
		}
		
		$variant = $this->model_catalog_ka_variant->getVariantByOptions($product['product_id'], $rec);
		if (empty($variant)) {
			$this->model_catalog_product->rebuildVariants($product['product_id']);
			$variant = $this->model_catalog_ka_variant->getVariantByOptions($product['product_id'], $rec);
		}
		
		if (empty($variant)) {
			$this->addImportMessage($this->product_mark . "Variants are not generated for unknown reason for productid=" . $product['product_id']);
			return false;
		}
		...
		//
		$rec = array(
			'model'    => $row[$cols['model']['column']],
		);
		
		//quantity
		if (isset($cols['quantity']['column']) && isset($row[$cols['quantity']['column']])) {
			$rec['quantity'] = $row[$cols['quantity']['column']];
		}
		
		//wheight
		if (isset($cols['weight']['column']) && isset($row[$cols['weight']['column']])) {
			$rec['weight']   = $row[$cols['weight']['column']];
		}
		
		// price
		if (isset($row[$cols['price']['column']]) && strlen($row[$cols['price']['column']])) {
			$rec['price'] = $this->kaformat->parsePrice($row[$cols['price']['column']]);
			if (!empty($this->params['price_multiplier'])) {
				$rec['price'] = $rec['price'] * $this->params['price_multiplier'];
			}
		}
		
		if (!empty($cols['image']['column'])) {
			$image = $this->getImageFile($row[$cols['image']['column']]);
			if ($image === false) {
				$this->addImportMessage($this->product_mark ."variant image cannot be saved - " . $this->lastError);
			} else {
				$rec['image'] = $image;
			}
		}
		
		$this->kadb->queryUpdate('ka_product_variants', $rec, "variant_id = '" . $variant['variant_id'] . "'");
	...
	*/
	...
	protected function deleteRecords() {
		...
		if ($this->params['update_mode'] == 'replace') {
			...
			if (!empty($this->params['matches']['ext_options'])
				|| !empty($this->params['matches']['options']))
			{
	...
	protected function process() {
		...
		while (1) {
			...
			foreach ($this->params['matches']['fields'] as $fk => $fv) {
				...
				if (is_array($fv['column'])) {
					$data[$fv['field']] = array();
					foreach ($fv['column'] as $column_index) {
						$data[$fv['field']][] = trim($row[$column_index]);
					}
				} else {
			...
			if (!empty($product_id) && !$this->isImportable($product_id)) {
				...
				if ($this->db->isKaInstalled('ka_variants')) {
					$this->saveVariants($row, $product, $delete_old, $is_first_product);
				}
			}
		}
	...
	*/
	...
	protected function getCustomProductDescriptionFields() {
		$default_fields = array('product_id', 'language_id', 'name', 'description', 'tag',
		...
		foreach ($qry->rows as $row) {
			...
			$field = array(
				'field' => $row['Field'],
				'name'  => $row['Field'],
				'descr' => 'Custom multi-language field. Type: ' . $row['Type'],
	...
	protected function getVariantColumns() {
		...
		$columns = array_merge($columns, array(				
			...
			'price' => array(
				'field' => 'price',
				'name'  => 'Price',
				'descr' => 'Regular variant price in primary currency (' . $this->config->get('config_currency') . ')',
			),
			'quantity' => array(
				'field' => 'quantity',
				'copy'  => true,
	...
	public function getFieldSets() {
		...
		$fields = array(
			...
			array(
				...
				'tip'      => 'This product import extension can find or create categories by name.<br/>If you need to import other category fields please take a look at "<a href="http://www.opencart.com/index.php?route=extension/extension/info&extension_id=22692" target="_blank">CSV Category Import</a>" extension.',
			),
			array(
				'field' => 'image',
			...
			array(
				...
				'descr' => "Relative paths to image files within 'image' directory or URL.",
				'tip'   => 'Multiple additional images are separated with a multi-value separator defined at <a target="_blank" href="index.php?route=ka_extensions/ka_product_import&token=' . $this->session->data['token'] . '">extension settings</a> page. See more information at <a target="_blank" href="https://www.ka-station.com/tickets/kb/faq.php?id=1">our help portal</a>.',
			),
			array(				
				'field' => 'manufacturer',
				'name'  => 'Manufacturer',
			...
			array(
				'field' => 'tax_class',
				'name'  => 'Tax class',
				'descr' => 'Existing tax class name from the <a target="_blank" href="' . $this->url->link('localisation/tax_class', 'token=' . $this->session->data['token'], true) . '">Taxes</a> page'
			),
			array(
				'field' => 'quantity',
				'copy'  => true,
			...
			'stock_status' => array(
				...
				'descr' => 'Existing stock status name from the <a target="_blank" href="' . $this->url->link('localisation/stock_status', 'token=' . $this->session->data['token'], true) . '">Stock Statuses</a> page'
			),
			array(
				'field' => 'status',
				'name'  => 'Status',
			...
			array(
				...
				'descr' => 'model identifier of the related product. Multiple values are allowed.',
			),
			'downloads' => array(
				'field' => 'downloads',
				'name'  => 'Downloads',
		...
		if (!empty($custom_fields)) {
			reset($custom_fields);
			$custom_fields[key($custom_fields)]['section'] = 'Custom Fields';
	...
	/*
		$sets    - sets array
		$matches - matched column names and product fields
		$columns - hash array of column names
	*/
					...
					if (is_array($matches[$sk][$f_key])) {
						...
						foreach ($matches[$sk][$f_key] as $mfk => $mfv) {
							if (isset($columns[$mfv])) {
								$sets[$sk][$f_idx]['column'][] = $columns[$mfv];
							}
						}
					} else {
