	...
	*/
	protected function getProduct($product_id, $columns) {
		...
		foreach ($columns['general'] as $ck => $cv) {
			...
			} elseif (!empty($field['is_product_description'])) {
			
				// MS Excel does not handle 'Carriarge Return' character properly in text fields
				//
				$product_description[$field['field']] = trim(str_replace("\r", "", $product_description[$field['field']]));
				$data['general'][$ck] = htmlspecialchars_decode($product_description[$field['field']]);
					...
					if (isset($pov['product_option_value'])) {						
						if (!empty($pov['product_option_value'])) {
							...
							foreach ($pov['product_option_value'] as $valk => $valv) {
								...
								$sub_option['ovalue_price']    = (($valv['price_prefix'] == '-') ? -1:1) * $this->kaformat->parsePrice($valv['price']);
	...
	protected function format($val, $type = 'text', $format = '') {
		if ($type == 'date' || $type == 'text' || $type == 'float') {
	...
	*/
	...
	protected function getHeader($columns) {
		...
		$sets = array(
			...
			'filter_groups' => array('filter group:'), 
	...
	public function getFieldSets($params) {
		...
		*/
		...
		if (!empty($params['separate_units'])) {
			$fields = array_merge($fields, $fields_units);
		}
		
		$fields_meta = array(
			'pd/meta_keyword' => array(
				'field' => 'meta_keyword',
		...
		$fields = array_merge($fields, $fields_meta);
