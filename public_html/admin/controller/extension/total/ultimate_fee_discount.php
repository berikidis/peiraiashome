...
class ControllerExtensionTotalUltimateFeeDiscount extends Controller {
	private $type = 'total';
	private $name = 'ultimate_fee_discount';
	...
	public function index() {
		...
		$data['rule_options'] = array(
			...
			'discount_criteria'			=> array('coupon', 'gift_voucher', 'reward_points'),
			'location_criteria'			=> array('address', 'city', 'country', 'distance', 'geo_zone', 'location_comparison', 'postcode', 'zone'),
			'order_criteria'			=> array('currency', 'custom_field', 'customer', 'customer_group', 'language', 'past_orders', 'payment_extension', 'shipping_cost', 'shipping_extension', 'shipping_rate', 'store'),
			'product_criteria'			=> array('attribute', 'attribute_group', 'category', 'manufacturer', 'option', 'product', 'product_group', 'other_product_data'),
		...
		$data['tax_class_array'] = array(0 => $data['text_none'], 'highest' => $data['text_highest_product_tax_rate']);
		...
		foreach (array('prediscounted', 'nondiscounted', 'ignoreoptions', 'taxed', 'total') as $total_value) {
		...
		$data['typeaheads'] = array('attribute', 'attribute_group', 'category', 'manufacturer', 'option', 'option_value', 'product');
		...
		foreach ($product_column_query->rows as $column) {
			$data['product_columns'][] = $column['Field'];
		}
		sort($data['product_columns']);
		...
		foreach (array('shipping', 'payment') as $extension_type) {
			...
			foreach ($extension_query->rows as $extension) {
				...
				$data[$extension_type . '_extension_array'][$extension['code']] = $extension_language['heading_title'];
			}
		}
		...
		$data['settings'][] = array(
			'type'		=> 'tabs',
			'tabs'		=> array('extension_settings', 'charges', 'charge_combinations', 'product_groups', 'rule_sets', 'testing_mode'),
		);
		$data['settings'][] = array(
		...
		foreach ($this->getTableRowNumbers($data, $table, $sortby) as $num => $rules) {
			...
			$charge_options['shipping_cost']				= $data['text_shipping_cost'];
			$charge_options['total']						= $data['text_total'];
			$charge_options['volume']						= $data['text_volume'];
			$charge_options['weight']						= $data['text_weight'];
			$charge_options['text_other_charges']			= '';
			$charge_options['buy_x_for_y']					= $data['text_buy_x_for_y'];
			$charge_options['buy_x_get_y']					= $data['text_buy_x_get_y'];
		...
		$data['settings'][] = array(
			'type'		=> 'html',
			'content'	=> '<div class="text-info text-center pad-bottom">' . $data['help_charge_combinations'] . '</div>',
		);
		$data['settings'][] = array(
			'key'		=> 'charge_combinations',
			'type'		=> 'heading',
