...
//karapuz (ka_csv_product_export.ocmod.xml) 
			if ($this->user->hasPermission('access', 'tool/ka_product_export')) {
				if (method_exists($this->db, 'isKaInstalled') && $this->db->isKaInstalled('ka_product_export')) {
					$tool[] = array(
						'name'	   => KaGlobal::t('CSV Product Export'),
						'href'     => $this->url->link('tool/ka_product_export', 'token=' . $this->session->data['token'], true),
						'children' => array()
					);
				}
			}
