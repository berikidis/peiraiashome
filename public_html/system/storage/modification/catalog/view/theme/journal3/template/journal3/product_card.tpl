...
<?php foreach ($products as $product): ?>
  ...
  <div class="product-layout <?php echo $j3->classes(\Journal3\Utils\Arr::get($product, 'classes')); ?> <?php echo $classes; ?>">
    <div class="product-thumb">
      ...
      <div class="name"><a href="<?php echo $product['href']; ?>" <?php if (isset($ee_tracking) && $ee_tracking && $ee_click) { ?>onclick="ee_product.click('<?php echo $product['product_id']; ?>', '<?php echo isset($product['ee_position']) ? $product['ee_position'] : ''; ?>', '<?php echo isset($ee_type) ? $ee_type : ''; ?>')"<?php } ?>><?php echo $product['name']; ?></a></div>
      <?php endif; ?>
      <div class="image">
        ...
        <a href="<?php echo $product['href']; ?>" <?php if (isset($ee_tracking) && $ee_tracking && $ee_click) { ?>onclick="ee_product.click('<?php echo $product['product_id']; ?>', '<?php echo isset($product['ee_position']) ? $product['ee_position'] : ''; ?>', '<?php echo isset($ee_type) ? $ee_type : ''; ?>')"<?php } ?> class="product-img <?php if ($product['second_thumb']): ?>has-second-image<?php endif; ?>">
          <div>
              ...
              <?php $index = 0; foreach ($product['extra_buttons'] as $id => $extra_button): $index++; ?>
                <a class="btn btn-extra btn-extra-<?php echo $id; ?>"
                  <?php if (($j3->settings->getIn($prefix . 'ExtraButtonDisplay' . $index, $context) == "icon") && ($j3->settings->getIn($prefix . 'ExtraButtonTooltipStatus', $context))): ?> data-toggle="tooltip" data-tooltip-class="<?php echo isset($module_id) ? 'module-products-' . $module_id . ' module-products-grid' : 'product-grid'; ?> extra-tooltip" data-placement="<?php echo $j3->settings->getIn($prefix . 'ExtraButtonTooltipPosition', $context); ?>" title="<?php echo $extra_button['label']; ?>" <?php endif; ?>
                   <?php if ($extra_button['action'] == 'quickbuy'): ?><?php if (isset($ee_tracking) && $ee_tracking && $ee_cart) { ?>data-eet-position="<?php echo isset($product['ee_position']) ? $product['ee_position'] : ''; ?>" data-eet-type="<?php echo isset($ee_type) ? $ee_type : ''; ?>"<?php } ?> onclick="cart.add('<?php echo $product['product_id']; ?>', $(this).closest('.product-thumb').find('.button-group input[name=\'quantity\']').val(), true);"<?php endif; ?>
