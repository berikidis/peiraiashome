...
<?php if (!function_exists('renderProducts')):
  function renderProducts($j3, $item, $context) {
    ...
    $ee_tracking = isset($item['ee_tracking']) ? $item['ee_tracking'] : false;
    $ee_impression = isset($item['ee_impression']) ? $item['ee_impression'] : false;
    $ee_click = isset($item['ee_click']) ? $item['ee_click'] : false;
    $ee_cart = isset($item['ee_cart']) ? $item['ee_cart'] : false;
    $ee_type = isset($item['ee_type']) ? $item['ee_type'] : false;

    if ($ee_tracking && $ee_impression) { ?>
    <script type="text/javascript"><!--
    $(document).ready(function() {
        setIntervalEE(function() {
            var ee_data = JSON.parse('<?php echo $item['ee_impression_data']; ?>');
        ...
        }, <?php echo $item['ee_ga_callback'] ? $item['ee_ga_callback'] : 0; ?>, <?php echo $item['ee_generate_cid'] ? $item['ee_generate_cid'] : 0; ?>);
    });
    //--></script>
    <?php }
