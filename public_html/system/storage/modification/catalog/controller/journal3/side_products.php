...
class ControllerJournal3SideProducts extends ModuleController {
	...
	protected function beforeRender() {
		...
		foreach ($this->settings['items'] as $key => $item) {
            ...
            /** EET Module */
            $this->settings['items'][$key]['ee_tracking'] = $this->config->get('ee_tracking_status');
            if ($this->settings['items'][$key]['ee_tracking']) {
                $this->settings['items'][$key]['ee_impression'] = $this->config->get('ee_tracking_impression_status');
                $this->settings['items'][$key]['ee_impression_log'] = $this->config->get('ee_tracking_log') ? $this->config->get('ee_tracking_impression_log') : false;
                $this->settings['items'][$key]['ee_click'] = $this->config->get('ee_tracking_click_status');
                $this->settings['items'][$key]['ee_cart'] = $this->config->get('ee_tracking_cart_status');
                $this->settings['items'][$key]['ee_ga_callback'] = $this->config->get('ee_tracking_advanced_settings') ? $this->config->get('ee_tracking_ga_callback') : 0;
                $this->settings['items'][$key]['ee_generate_cid'] = $this->config->get('ee_tracking_advanced_settings') ? $this->config->get('ee_tracking_generate_cid') : 0;

                $this->settings['items'][$key]['ee_type'] = 'module_' . strtolower($item['filter']['preset']);
                $ee_data = array('type' => $this->settings['items'][$key]['ee_type']);
                $ee_position = 0;
                foreach ($products as $p_id => $result) {
                    $ee_data['products'][] = $result['product_id'];
                    $products[$p_id]['ee_position'] = ++$ee_position;

                }
                $this->settings['items'][$key]['ee_impression_data'] = json_encode($ee_data);
            }
            /** EET Module */
