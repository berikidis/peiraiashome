...
class ControllerJournal3Manufacturers extends ModuleController {
	...
	protected function parseItemSettings($parser, $index) {
        ...
        /** EET Module */
        $ee_position = 1;
        $ee_tracking['ee_tracking'] = $this->config->get('ee_tracking_status');
        if ($ee_tracking['ee_tracking'] && $results) {
            $ee_tracking['ee_promotion'] = $this->config->get('ee_tracking_promotion_status');
            $ee_tracking['ee_promotion_log'] = $this->config->get('ee_tracking_log') ? $this->config->get('ee_tracking_promotion_log') : false;
            $ee_tracking['ee_ga_callback'] = $this->config->get('ee_tracking_advanced_settings') ? $this->config->get('ee_tracking_ga_callback') : 0;
            $ee_tracking['ee_generate_cid'] = $this->config->get('ee_tracking_advanced_settings') ? $this->config->get('ee_tracking_generate_cid') : 0;
            $ee_list = array();
            foreach ($results as $result) {
                $ee_list[] = $result['manufacturer_id'];
            }
            $ee_tracking['ee_data'] = json_encode(array('banner_id' => 'jbrand_' . $this->item_id, 'list' => $ee_list));
        }
        /** EET Module */
		...
		foreach ($results as $result) {
			...
			$manufacturers[$result['manufacturer_id']] = array(
                'ee_banner_id'    => 'jbrand_' . $this->item_id . '_' . $result['manufacturer_id'],
		...
		return array(
			...
			'ee_tracking' => isset($ee_tracking) ? $ee_tracking : false,
