...
class ModelExtensionModuleEeTracking extends Model {
    ...
    protected function getGlobalFields() {
        $fields = array();
        $fields['v'] = 1;
        $fields['tid'] = $this->config->get('ee_tracking_tracking_id');
        $fields['t'] = 'event';
        ...
        if ($client_id && $data) {
            ...
            if (count($data['products']) > 26) {
                ...
                foreach ($batches as $batch) {
                    ...
                    $response = $this->setCurlRequest($product_fields, $this->config->get('ee_tracking_impression_debug'));
                    if ($this->config->get('ee_tracking_log') && $this->config->get('ee_tracking_impression_log')) {
                        $log = 'LIST VIEW';
                        $log .= $this->config->get('ee_tracking_impression_debug') ? ' / mode: debug' : ' / mode: release';
                        $log .= ' / list: ' . $fields['il1nm'] . $response;
                        if ($this->config->get('ee_tracking_extended_log')) {
                ...
                $response = $this->setCurlRequest($fields, $this->config->get('ee_tracking_impression_debug'));
                if ($this->config->get('ee_tracking_log') && $this->config->get('ee_tracking_impression_log')) {
                    $log = 'LIST VIEW';
                    $log .= $this->config->get('ee_tracking_impression_debug') ? ' / mode: debug' : ' / mode: release';
                    $log .= ' / list: ' . $fields['il1nm'] . $response;
    ...
    public function detail($data) {
        ...
        if ($client_id && $data) {
            ...
            $fields = array_merge($fields, $product_fields);
            $response = $this->setCurlRequest($fields, $this->config->get('ee_tracking_detail_debug'));
            if ($this->config->get('ee_tracking_log') && $this->config->get('ee_tracking_detail_log')) {
                $product_info = $this->getProductDescription($data['product_id'], $settings['language_id']);
                $log = 'PRODUCT DETAIL VIEW';
                $log .= $this->config->get('ee_tracking_detail_debug') ? ' / mode: debug' : ' / mode: release';
    ...
    public function click($data) {
        ...
        if ($client_id && $data) {
            ...
            $fields = array_merge($fields, $product_fields);
            $response = $this->setCurlRequest($fields, $this->config->get('ee_tracking_click_debug'));
            if ($this->config->get('ee_tracking_log') && $this->config->get('ee_tracking_click_log')) {
                $log = 'PRODUCT CLICK';
                $log .= $this->config->get('ee_tracking_click_debug') ? ' / mode: debug' : ' / mode: release';
    ...
    public function quickAddToCart($data) {
        ...
        if ($client_id && $data) {
            ...
            $fields = array_merge($fields, $product_fields);
            $response = $this->setCurlRequest($fields, $this->config->get('ee_tracking_cart_debug'));
            if ($this->config->get('ee_tracking_log') && $this->config->get('ee_tracking_cart_log')) {
                $log = 'ADD TO CART';
                $log .= $this->config->get('ee_tracking_cart_debug') ? ' / mode: debug' : ' / mode: release';
    ...
    public function checkout($data) {
        ...
        if ($client_id && $data) {
            ...
            if ($cart_products && $this->config->get('ee_tracking_checkout_status')) {
                if ($data['step_option'] == 'custom checkout') {
                    $status = true;
                    foreach ($this->config->get('ee_tracking_checkout_url') as $key => $value) {
                        if (strpos($data['url'], $value) !== false && $this->config->get('config_language_id') == $key) {
                            $status = false;
                        }
                    }
                    if (!$this->config->get('ee_tracking_checkout_custom') || $status) {
                        return false;
                    }
                }
                ...
                $response = $this->setCurlRequest($fields, $this->config->get('ee_tracking_checkout_debug'));
                if ($this->config->get('ee_tracking_log') && $this->config->get('ee_tracking_checkout_log')) {
                    $log = 'CHECKOUT';
                    $log .= $this->config->get('ee_tracking_checkout_debug') ? ' / mode: debug' : ' / mode: release';
    ...
    public function transaction($order_id, $order_status_id) {
        if ($this->config->get('ee_tracking_admin_tracking') &&  isset($this->session->data['user_id']) && $this->session->data['user_id']) {
            return false;
        }
        $fields = array();
    ...
    public function refund($data) {
        ...
        if (isset($data['order_id'])) {
            $fields = $this->getGlobalFields();
            $client_id = $this->getOrderToClient($data['order_id'], $this->config->get('ee_tracking_refund_debug'), true);
            if ($client_id) {
                ...
                $response = $this->setCurlRequest($fields, $this->config->get('ee_tracking_refund_debug'));
                if ($this->config->get('ee_tracking_log') && $this->config->get('ee_tracking_refund_log')) {
                    $log = 'REFUND';
                    $log .= $this->config->get('ee_tracking_refund_debug') ? ' / mode: debug' : ' / mode: release';
    ...
    public function promotion($data) {
        ...
        if ($client_id) {
            ...
            $banner_info = array();
            $banners = array();
            if (strpos($data['banner_id'], 'jbrand_') !== false) {
                $banner_info['name'] = 'Brands Module';
                $parts = explode('_', $data['banner_id']);
                $module_id = isset($parts[1]) ? $parts[1] : 0;
                $this->load->model('catalog/manufacturer');
                foreach ($data['list'] as $manufacturer_id) {
                    $manufacturer_info = $this->model_catalog_manufacturer->getManufacturer($manufacturer_id);
                    $banners[] = array(
                        'banner_id' => $module_id,
                        'banner_image_id' => $manufacturer_id,
                        'title' => $manufacturer_info['name'],
                    );
                }
            } elseif (strpos($data['banner_id'], 'jbanner_') !== false || strpos($data['banner_id'], 'jmslider_') !== false) {
                $this->load->model('journal3/module');
                $parts = explode('_', $data['banner_id']);
                $module_id = isset($parts[1]) ? $parts[1] : 0;
                if ($parts[0] == 'jbanner') {
                    $module_info = $this->model_journal3_module->get($module_id, 'banners');
                    $banner_info['name'] = isset($module_info['general']['name']) ? $module_info['general']['name'] : 'Banner Module';
                } else if ($parts[0] == 'jmslider') {
                    $module_info = $this->model_journal3_module->get($module_id, 'master_slider');
                    $banner_info['name'] = isset($module_info['general']['name']) ? $module_info['general']['name'] : 'Master Slider Module';
                }
                $image_id = 0;
                if (isset($module_info['items']) && $module_info['items']){
                    foreach ($module_info['items'] as $item) {
                        $banners[] = array(
                            'banner_id' => $module_id,
            ...
            if ($banner_info && $banners) {
                ...
                $response = $this->setCurlRequest($fields, $this->config->get('ee_tracking_promotion_debug'));
                if ($this->config->get('ee_tracking_log') && $this->config->get('ee_tracking_promotion_log')) {
                    $log = 'PROMOTION IMPRESSION';
                    $log .= $this->config->get('ee_tracking_promotion_debug') ? ' / mode: debug' : ' / mode: release';
    ...
    public function promotionClick($data) {
        ...
        if ($client_id) {
            ...
            if (strpos($data['banner_id'], 'jbrand_') !== false) {
                ...
                $parts = explode('_', $data['banner_id']);
                $module_id = isset($parts[1]) ? $parts[1] : 0;
                $manufacturer_id = isset($parts[2]) ? $parts[2] : 0;
                $this->load->model('catalog/manufacturer');
                $manufacturer_info = $this->model_catalog_manufacturer->getManufacturer($manufacturer_id);
                $banners[] = array(
                    'banner_id' => 0,
                ...
                if ($this->config->get('ee_tracking_promotion_debug')) {
                    $fields['promo1a'] = 'click';
                } else {
                    $fields['promoa'] = 'click';
                }
                $fields['promo1id'] = 'PROMO_' . $module_id . '_' . $manufacturer_id;
                $fields['promo1nm'] = $banner_info['name'];
                $fields['promo1cr'] = isset($manufacturer_info['name']) ? $manufacturer_info['name'] : '';
                $fields['promo1ps'] = 'banner_slot' . $data['position'];
                $data['position'] = 0;

            } elseif (strpos($data['banner_id'], 'jbanner-') !== false || strpos($data['banner_id'], 'jmslider-') !== false) {
                $this->load->model('journal3/module');
                $parts = explode('-', $data['banner_id']);
                $module_id = isset($parts[1]) ? $parts[1] : 0;
                $image_id = isset($parts[2]) ? $parts[2] : 0;
                if ($parts[0] == 'jbanner') {
                    $module_info = $this->model_journal3_module->get($module_id, 'banners');
                    $banner_info['name'] = isset($module_info['general']['name']) ? $module_info['general']['name'] : 'Banner Module';
                } else if ($parts[0] == 'jmslider') {
    ...
    protected function getDataByUrl($page_url, $product_id = 0, $position = 0) {
        ...
        if (isset($url_data['host']) && isset($url_data['path']) && isset($this->request->server['HTTP_HOST']) && strpos($this->request->server['HTTP_HOST'], $url_data['host']) !== false || strpos(HTTP_SERVER, $url_data['host']) !== false) {
            ...
            if ($query && isset($query['route']) || isset($query['product_id'])) {
                ...
                $db_query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "url_alias WHERE keyword = '" . $this->db->escape($keyword) . "'");
    ...
    protected function getSettings() {
        if ($this->config->get('ee_tracking_advanced_settings')) {
            if ($this->config->get('ee_tracking_language_id')) {
                $settings['language_id'] = $this->config->get('ee_tracking_language_id');
            } else {
                $settings['language_id'] = $this->config->get('config_language_id');
            }
            if ($this->config->get('ee_tracking_currency')) {
                $settings['currency'] = $this->config->get('ee_tracking_currency');
            } else {
                $settings['currency'] = isset($this->session->data['currency']) ? $this->session->data['currency'] : '';
            }
            if ($this->config->get('ee_tracking_affiliation')) {
                $settings['affiliation'] = $this->config->get('ee_tracking_affiliation');
            } else {
                $settings['affiliation'] = $this->config->get('config_name');
            }
            $settings['tax'] = $this->config->get('ee_tracking_tax');
            if ($this->config->get('ee_tracking_product_id')) {
                $settings['product_id'] = $this->config->get('ee_tracking_product_id');
            } else {
                $settings['product_id'] = 'product_id';
            }
            $settings['product_category'] = $this->config->get('ee_tracking_product_category');
            $settings['compatibility'] = $this->config->get('ee_tracking_compatibility');
        } else {
            $settings['language_id'] = $this->config->get('config_language_id');
            $settings['currency'] = isset($this->session->data['currency']) ? $this->session->data['currency'] : '';
            $settings['affiliation'] = $this->config->get('config_name');
        ...
        $settings['custom_dimension'] = $this->config->get('ee_tracking_custom_dimension');
    ...
    protected function getSettingsByStore($data) {
        ...
        if ($this->config->get('ee_tracking_multistore')) {
            $settings['store_id'] = $data['store_id'];
        } else {
            $settings['store_id'] = 0;
        }
        $this->load->model('setting/setting');
        $module_info = $this->model_setting_setting->getSetting('ee_tracking', $settings['store_id']);
        if ($module_info) {
            foreach ($module_info as $key => $item) {
                $settings[str_replace("ee_tracking_", "", $key)] = $item;
            }
    ...
    public function getClientId() {
        if ($this->config->get('ee_tracking_bot_filter') && isset($this->request->server['HTTP_USER_AGENT']) && preg_match('/' . $this->config->get('ee_tracking_bot_filter') . '/i', $this->request->server['HTTP_USER_AGENT'])) {
            return false;
        }
        if ($this->config->get('ee_tracking_admin_tracking') &&  isset($this->session->data['user_id']) && $this->session->data['user_id']) {
            return false;
        }
        if ($this->config->get('ee_tracking_ip_filter')) {
            $ip_address = $this->getUserIP();
            $ip_blocked = preg_replace('~\r?\n~', "\n", $this->config->get('ee_tracking_ip_filter'));
            $ips = explode("\n", $ip_blocked);
            foreach ($ips as $ip) {
                $ip = trim($ip);
                if ($ip_address == $ip) {
        ...
        } elseif ($this->config->get('ee_tracking_advanced_settings') && $this->config->get('ee_tracking_generate_cid')) {
    ...
    public function addLog($data) {
        if ($data) {
            ...
            $log = new Log('ee_tracking' . $store_id . '.log');
            $log->write($data);
        }
    }
